...
public final class RSSExpandedReader extends AbstractRSSReader {
	...
	private static final int[][] FINDER_PATTERNS = { { 1, 8, 4, 1 }, // A
			{ 3, 6, 4, 1 }, // B
			{ 3, 4, 6, 1 }, // C
			{ 3, 2, 8, 1 }, // D
			{ 2, 6, 5, 1 }, // E
	...
	private static final int[][] WEIGHTS = { { 1, 3, 9, 27, 81, 32, 96, 77 },
			{ 20, 60, 180, 118, 143, 7, 21, 63 }, { 189, 145, 13, 39, 117, 140, 209, 205 },
			{ 193, 157, 49, 147, 19, 57, 171, 91 }, { 62, 186, 136, 197, 169, 85, 44, 132 },
			{ 185, 133, 188, 142, 4, 12, 36, 108 }, { 113, 128, 173, 97, 80, 29, 87, 50 },
			{ 150, 28, 84, 41, 123, 158, 52, 156 }, { 46, 138, 203, 187, 139, 206, 196, 166 },
			{ 76, 17, 51, 153, 37, 111, 122, 155 }, { 43, 129, 176, 106, 107, 110, 119, 146 },
			{ 16, 48, 144, 10, 30, 90, 59, 177 }, { 109, 116, 137, 200, 178, 112, 125, 164 },
			{ 70, 210, 208, 202, 184, 130, 179, 115 }, { 134, 191, 151, 31, 93, 68, 204, 190 },
			{ 148, 22, 66, 198, 172, 94, 71, 2 }, { 6, 18, 54, 162, 64, 192, 154, 40 },
			{ 120, 149, 25, 75, 14, 42, 126, 167 }, { 79, 26, 78, 23, 69, 207, 199, 175 },
			{ 103, 98, 83, 38, 114, 131, 182, 124 }, { 161, 61, 183, 127, 170, 88, 53, 159 },
	...
	private static final int[][] FINDER_PATTERN_SEQUENCES = {
			...
			{ FINDER_PAT_A, FINDER_PAT_E, FINDER_PAT_B, FINDER_PAT_D, FINDER_PAT_E, FINDER_PAT_F,
					FINDER_PAT_F },
			{ FINDER_PAT_A, FINDER_PAT_A, FINDER_PAT_B, FINDER_PAT_B, FINDER_PAT_C, FINDER_PAT_C,
					FINDER_PAT_D, FINDER_PAT_D },
			{ FINDER_PAT_A, FINDER_PAT_A, FINDER_PAT_B, FINDER_PAT_B, FINDER_PAT_C, FINDER_PAT_C,
					FINDER_PAT_D, FINDER_PAT_E, FINDER_PAT_E },
			{ FINDER_PAT_A, FINDER_PAT_A, FINDER_PAT_B, FINDER_PAT_B, FINDER_PAT_C, FINDER_PAT_C,
					FINDER_PAT_D, FINDER_PAT_E, FINDER_PAT_F, FINDER_PAT_F },
			{ FINDER_PAT_A, FINDER_PAT_A, FINDER_PAT_B, FINDER_PAT_B, FINDER_PAT_C, FINDER_PAT_D,
					FINDER_PAT_D, FINDER_PAT_E, FINDER_PAT_E, FINDER_PAT_F, FINDER_PAT_F }, };
	...
	@Override public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints)
	...
	List<ExpandedPair> decodeRow2pairs(int rowNumber, BitArray row) throws NotFoundException {
		while (true) {
			...
			if (nextPair.mayBeLast()) {
				if (checkChecksum()) return pairs;
				if (nextPair.mustBeLast()) throw NotFoundException.getNotFoundInstance();
			}
		}
	}
	...
	private static Result constructResult(List<ExpandedPair> pairs) throws NotFoundException {
		...
		return new Result(resultingString, null, new ResultPoint[] { firstPoints[0], firstPoints[1],
				lastPoints[0], lastPoints[1] }, BarcodeFormat.RSS_EXPANDED);
	}
	...
	ExpandedPair retrieveNextPair(BitArray row, List<ExpandedPair> previousPairs, int rowNumber)
		...
		do {
			...
			if (pattern == null) forcedOffset = getNextSecondBar(row, startEnd[0]);
	...
	private boolean checkPairSequence(List<ExpandedPair> previousPairs, FinderPattern pattern)
		...
		for (int pos = 0; pos < previousPairs.size(); ++pos)
			currentSequence[pos] = previousPairs.get(pos).getFinderPattern().getValue();

		currentSequence[currentSequenceLength - 1] = pattern.getValue();
		...
		for (int[] validSequence : FINDER_PATTERN_SEQUENCES)
			if (validSequence.length >= currentSequenceLength) {
				...
				if (valid) return currentSequenceLength == validSequence.length;
			}
	...
	private void findNextPair(BitArray row, List<ExpandedPair> previousPairs, int forcedOffset)
		...
		else if (previousPairs.isEmpty()) rowOffset = 0;
		else {
		...
		for (int x = rowOffset; x < width; x++)
			if (row.get(x) ^ isWhite) counters[counterPosition]++;
			else {
				if (counterPosition == 3) {
					if (searchingEvenPair) reverseCounters(counters);
	...
	private FinderPattern parseFoundFinderPattern(BitArray row, int rowNumber, boolean oddPattern) {
		...
		if (oddPattern) {
			// If pattern number is odd, we need to locate element 1 *before* the
			// current block.
			...
			// If pattern number is even, the pattern is reversed, so we need to
			// locate element 1 *after* the current block.
	...
	DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern, boolean isOddPattern,
		...
		if (leftChar) recordPatternInReverse(row, pattern.getStartEnd()[0], counters);
		else {
		...
		int numModules = 17; // left and right data characters have all the same
													// length
	...
	private static boolean isNotA1left(FinderPattern pattern, boolean isOddPattern, boolean leftChar) {
		// A1: pattern.getValue is 0 (A), and it's an oddPattern, and it is a left
		// char
	...
	private void adjustOddEvenCounts(int numModules) throws NotFoundException {
		...
		if (mismatch == 1) {
			if (oddParityBad) {
				if (evenParityBad) throw NotFoundException.getNotFoundInstance();
				decrementOdd = true;
			} else {
				if (!evenParityBad) throw NotFoundException.getNotFoundInstance();
				decrementEven = true;
			...
			} else if (evenParityBad) throw NotFoundException.getNotFoundInstance();
		} else throw NotFoundException.getNotFoundInstance();

		if (incrementOdd) {
			if (decrementOdd) throw NotFoundException.getNotFoundInstance();
			increment(getOddCounts(), getOddRoundingErrors());
		}
		if (decrementOdd) decrement(getOddCounts(), getOddRoundingErrors());
		if (incrementEven) {
			if (decrementEven) throw NotFoundException.getNotFoundInstance();
			increment(getEvenCounts(), getOddRoundingErrors());
		}
		if (decrementEven) decrement(getEvenCounts(), getEvenRoundingErrors());
	}
}
